...
<template>
  <!--begin::Step-->
  <div class="step-div">
    <!-- <el-steps :active="stepInfo.active" align-center class="step">
      <template v-for="(item, index) in stepInfo.steps" :key="index">
        <el-step
          :title="item.title"
          description=""
    ...
    <!-- <el-steps :space="200" :active="1" finish-status="success">
      <el-step title="Done" status="wait"/>
      <el-step title="Processing" status="process"/>
      <el-step title="Step 3" status="finish"/>
      <el-step title="Step 4" status="error"/>
      <el-step title="Step 5" status="success"/>
    </el-steps> -->
  </div>
  <!--end::Step-->
  <!--begin::Basic info-->
...
onMounted(async () => {
  //初始化数据
  if (isEdit.value) {
    const response = await GroupReceiptSelfService.get(Number(route.params.id));
...
//   ReceiptServiceImpl.review(currentDealReceipt.value.receipt.id, 1).then(() => {
//     //更新列表数据
...
//     const response = await GroupReceiptServiceImpl.toPaid(
