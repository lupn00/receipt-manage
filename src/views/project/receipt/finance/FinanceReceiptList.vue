  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <el-table-column property="employeeName" label="提交人" width="130" />
        <!-- <el-table-column
...
import { GroupReceiptFinanceServiceImpl as GroupReceiptFinanceService } from "@/core/services/project/ReceiptFinanceService";
import {
  type IGroupReceipt,
  receiptStatus2Str,
  receiptStatus2Tag,
  ...
  (newVal) => {
    ...
    switch (statusStr) {
      case "wait-verify":
        ...
        break;
      case "rejected":
        //已驳回 只可修改审核不通过的报销单内容，不可删除，不可新增报销单
        status.value = 3;
        break;
      case "passed":
        //审批通过 不可修改，不可删除
...
const queryMyTask = async () => {
  const response = await GroupReceiptFinanceService.queryMyTask();
...
const queryTableData = async (qc: IQueryCondition) => {
  const response = await GroupReceiptFinanceService.pageQuery(qc);
  const { limit, total } = response.data.obj;
  pager.value = { limit, total };
  tableData.value = response.data.obj.list;
};
...
const viewGroupReceipt = (index, row) => {
  router.push({ name: "finance-receipt-view", params: { id: `${row.id}` } });
};
const handleGroupReceiptDelete = async (index, row) => {
  const confirmResult = await confirmAlert(`确定要删除记录吗？`);
  if (confirmResult.isConfirmed) {
    const response = await GroupReceiptFinanceService.delete(row.id);
